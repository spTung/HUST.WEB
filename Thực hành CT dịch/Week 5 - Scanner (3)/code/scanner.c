...
Token* readIdentKeyword(void) { // a-z A-Z _ 0-9
...
Token* readNumber(void) { // Integer + Float
  int ln, cn;
...
Token* getToken(void) {
  ...
  switch (charCodes[currentChar]) {
    ...
    case CHAR_LETTER: return readIdentKeyword(); // a-z A-Z _

    case CHAR_DIGIT: return readNumber(); // 0-9
    ...
    case CHAR_OBRE: // {
      token = makeToken(SB_OPEN_BRACE, lineNo, colNo);
      readChar();
      return token;

    case CHAR_CBRE: // }
      token = makeToken(SB_CLOSE_BRACE, lineNo, colNo);
      readChar();
      return token;
...
void printToken(Token *token) {
  ...
  case SB_ASSIGN_DEVIDE: printf("SB_ASSIGN_DEVIDE\n"); break;
  // More 2
  ...
  case SB_OPEN_BRACE: printf("SB_OPEN_BRACE\n"); break;
  case SB_CLOSE_BRACE: printf("SB_CLOSE_BRACE\n"); break;
  }
}
