...
void compileConstDecls(void) { // DONE
...
void compileConstDecl(void) { // DONE
...
void compileTypeDecls(void) { // DONEE
...
void compileTypeDecl(void) { // DONE
...
void compileVarDecls(void) { // DONEE
...
void compileSubDecls(void) { // DONEE
...
void compileFuncDecl(void) { // DONE
  /* 
      FUNCTION foo (a:INTEGER; VAR b:CHAR) : INTEGER; {block};
      FUNCTION bar : INTEGER; {block};
  */
...
void compileProcDecl(void) { // DONE
  /*
      PROCEDURE foo (a:INTEGER; VAR b:CHAR); {block};
      PROCEDURE bar; {block};
  */
...
void compileUnsignedConstant(void) { // DONE
  switch (lookAhead->tokenType) {
...
void compileConstant(void) { // DONE
  /*
    3, 'a', +ident, -ident, +3, -3
  */
  switch (lookAhead->tokenType) {
  // FIRST(Constant) = {SB_PLUS, SB_MINUX, TK_CHAR, TK_NUMBER, TK_IDENT}
  case SB_PLUS:
    eat(SB_PLUS);
    compileConstant2();
    break;
...
void compileConstant2(void) { // DONE
  switch (lookAhead->tokenType) {
  // FIRST(Constant2) = {TK_IDENT, TK_NUMBER}
  case TK_IDENT:
    eat(TK_IDENT);
    break;
  case TK_NUMBER:
...
void compileType(void) { // DONE
  switch (lookAhead->tokenType) {
  // FIRST(Type) = {KW_INTEGER, KW_CHAR, TK_IDENT, KW_ARRAY}
  case KW_INTEGER:
    eat(KW_INTEGER);
    break;
  case KW_CHAR:
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    break;
  case KW_ARRAY: // ARRAY(.5.) OF INTEGER/CHAR/abc
    eat(KW_ARRAY);
    eat(SB_LSEL);
    eat(TK_NUMBER);
    eat(SB_RSEL);
...
void compileBasicType(void) { // DONE
  switch (lookAhead->tokenType) {
  // FIRST(BasicType) = {KW_INTEGER, KW_CHAR}
  case KW_INTEGER:
    eat(KW_INTEGER);
    break;
  case KW_CHAR:
...
void compileParams2(void) { // DONEE
  switch (lookAhead->tokenType) {
...
void compileStatements(void) { // DONEEE
  compileStatement();
  compileStatements2();
}

void compileStatements2(void) { // DONEE
  switch (lookAhead->tokenType) {
...
void compileAssignSt(void) { // DONE
  ...
  // Indexes ::= SB_LSEL Expression SB_RSEL Indexes|ε
...
void compileArguments(void) { // DONEE
  switch (lookAhead->tokenType) {
...
void compileArguments2(void) { // DONEE
...
void compileCondition(void) { // DONEEE
  compileExpression();
  compileCondition2();
}

void compileCondition2(void) { // DONE
  switch (lookAhead->tokenType) {
...
void compileExpression(void) { // DONE
...
void compileExpression2(void) { // DONEEE
  compileTerm();
  compileExpression3();
}

void compileExpression3(void) { // DONEE
  switch (lookAhead->tokenType) {
...
void compileTerm(void) { // DONEEE
  compileFactor();
  compileTerm2();
}

void compileTerm2(void) { // DONEE
  switch (lookAhead->tokenType) {
  // FIRST(Term2) = {SB_TIMES, SB_SLASH, ε}
  case SB_TIMES:
    eat(SB_TIMES);
    compileFactor();
    compileTerm2();
...
void compileFactor(void) { // DONE
  switch (lookAhead->tokenType) {
  // FIRST(Factor) = {TK_NUMBER, TK_IDENT, TK_CHAR, SB_LPAR}
  case TK_NUMBER:
  case TK_CHAR:
...
void compileIndexes(void) { // DONEE
  // FIRST(Indexes) = {SB_LSEL, ε}
  if (lookAhead->tokenType == SB_LSEL) {
    eat(SB_LSEL);
    compileExpression();
    eat(SB_RSEL);
